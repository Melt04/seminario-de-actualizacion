...
const authenticateRoute = new Route("POST", new RegExp("^/authenticate$"), async (req, res) => {
  try {
    res.statusCode = 200;
    res.write(
      JSON.stringify({
...
const userIsAuthorized = new Route("POST", new RegExp("^/authorized/user$"), async (req, res) => {
  try {
...
const authorizeAccessToResource = new Route("POST", new RegExp("^/authorized/resource$"), async (req, res) => {
  try {
    body = await getBodyFromRequest(req);
    ...
    res.statusCode = 201;
    res.write(JSON.stringify({ message: "Access to Resource created", error: false }));
    res.end();
  } catch (e) {
    console.log(e);
    res.statusCode = 500;
    res.write(JSON.stringify({ message: "Failed to create access", error: true }));

    res.end();
  }
});

const authorizeGroupToAccess = new Route("POST", new RegExp("^/authorized/group$"), async (req, res) => {
  try {
    body = await getBodyFromRequest(req);
